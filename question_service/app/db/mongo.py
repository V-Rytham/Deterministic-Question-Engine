...
def ensure_indexes() -> None:
    ...
    db.book_facts.create_index([("isbn", 1), ("chapter", 1), ("position", 1)])

    db.entity_bank.create_index([("book_id", 1), ("entity", 1)], unique=True)
    db.entity_bank.create_index([("isbn", 1), ("entity", 1)])
    db.entity_bank.create_index([("entity", 1), ("entity_type", 1)])
    db.entity_bank.create_index([("book_id", 1), ("entity_type", 1), ("frequency", -1)])

    db.book_questions.create_index([("book_id", 1)])
    db.book_questions.create_index([("isbn", 1)])
